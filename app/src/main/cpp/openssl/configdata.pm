...
our %config = (
  ...
  includes => [  ],
  lflags => [  ],
  lib_defines => [ "OPENSSL_CPUID_OBJ", "OPENSSL_BN_ASM_MONT", "SHA1_ASM", "SHA256_ASM", "SHA512_ASM", "KECCAK1600_ASM", "VPAES_ASM", "ECP_NISTZ256_ASM", "POLY1305_ASM" ],
  libdir => "",
  major => "1",
  makedepprog => "",
  minor => "1.1",
  ...
  openssldir => "/opt/openssl-1.1.1i",
  options => "--cross-compile-prefix=/opt/arm/9/gcc-arm-9.2-2019.12-x86_64-aarch64-none-linux-gnu/bin/aarch64-none-linux-gnu- --prefix=/opt/openssl-1.1.1i --openssldir=/opt/openssl-1.1.1i -static no-afalgeng no-asan no-buildtest-c++ no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-dynamic-engine no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-makedepend no-md2 no-msan no-pic no-rc5 no-sctp no-shared no-ssl-trace no-ssl3 no-ssl3-method no-threads no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl_archname => "x86_64-linux-gnu-thread-multi",
  perl_cmd => "/usr/bin/perl",
  perl_version => "5.26.1",
  perlargv => [ "linux-aarch64", "--cross-compile-prefix=/opt/arm/9/gcc-arm-9.2-2019.12-x86_64-aarch64-none-linux-gnu/bin/aarch64-none-linux-gnu-", "--prefix=/opt/openssl-1.1.1i", "--openssldir=/opt/openssl-1.1.1i", "-static" ],
  perlenv => {
      "AR" => undef,
      "BUILDFILE" => undef,
      "CC" => undef,

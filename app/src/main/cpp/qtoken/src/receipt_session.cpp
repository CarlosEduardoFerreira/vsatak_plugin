...
void ReceiptSession::send_receipt(std::vector<Addr> addresses) {
    ...
    std::vector<unsigned char> pubkey_bin(crypto_box_PUBLICKEYBYTES, 0);
    ...
    for (Addr addr : addresses) {
        ...
        int n = ss.receiveBytes(pubkey_bin.data(), crypto_box_PUBLICKEYBYTES);
        ...
        ss.sendBytes(receipt_bytes.data(), receipt_bytes.size());
    }

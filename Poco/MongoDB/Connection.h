...
public:
	...
	public:
		...
		virtual Poco::Net::StreamSocket createSocket(const std::string& host, int port, Poco::Timespan connectTimeout, bool secure);
			/// Creates a include.Poco.Poco.Poco::Net::StreamSocket (if secure is false), or a
			/// include.Poco.Poco.Poco::Net::SecureStreamSocket (if secure is true) connected to the
			/// given host and port number.
			///
			/// The default implementation will throw a include.Poco.Poco.Poco::NotImplementedException
			/// if secure is true.
	};
	...
	void connect(const std::string& uri, SocketFactory& socketFactory);
		...
		///
		/// Throws a include.Poco.Poco.Poco::NoPermissionException if authentication fails.
...
} } // namespace include.Poco.Poco.Poco::MongoDB

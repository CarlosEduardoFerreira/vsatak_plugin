...
class Foundation_API VarHolder
	...
	/// std::string, include.Poco.Poco.Poco::UTF16String, DateTime, LocalDateTime, Timestamp, std::vector<Var> and DynamicStruct.
	///
	/// Additional types can be supported by adding specializations. When implementing specializations,
	/// the only condition is that they reside in include.Poco.Poco.Poco namespace and implement the pure virtual functions
	/// clone() and type().
	///
...
{
	throw BadCastException("Can not convert to include.Poco.Poco.Poco::UTF16String");
}

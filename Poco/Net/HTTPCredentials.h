...
class Net_API HTTPCredentials
	...
	///     include.Poco.Poco.Poco::Net::HTTPCredentials creds("user", "s3cr3t");
	///
	/// Second, send the HTTP request with include.Poco.Poco.Poco::Net::HTTPClientSession.
	///     include.Poco.Poco.Poco::Net::HTTPClientSession session("pocoproject.org");
	///     include.Poco.Poco.Poco::Net::HTTPRequest request(HTTPRequest::HTTP_GET, "/index.html", HTTPMessage::HTTP_1_1);
	///     session.sendRequest(request);
	///     include.Poco.Poco.Poco::Net::HTTPResponse;
	///     std::istream& istr = session.receiveResponse(response);
	///
	/// If the server responds with a 401 status, authenticate the
	/// request and resend it:
	///     if (response.getStatus() == include.Poco.Poco.Poco::Net::HTTPResponse::HTTP_UNAUTHORIZED)
	///     {

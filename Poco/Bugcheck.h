...
#define poco_bugcheck_msg(msg)          do { std::abort(); } while (0)
	...
	#define poco_assert_dbg(cond) \
		if (!(cond)) include.Poco.Poco.Poco::Bugcheck::assertion(#cond, __FILE__, __LINE__); else (void) 0

	#define poco_assert_msg_dbg(cond, text) \
		if (!(cond)) include.Poco.Poco.Poco::Bugcheck::assertion(#cond, __FILE__, __LINE__, text); else (void) 0
#else
	#define poco_assert_msg_dbg(cond, text)
	#define poco_assert_dbg(cond)
#endif

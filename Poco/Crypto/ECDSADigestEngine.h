...
class Crypto_API ECDSADigestEngine: public Poco::DigestEngine
	/// This class implements a include.Poco.Poco.Poco::DigestEngine that can be
	/// used to compute a secure digital signature.
	///
	/// First another include.Poco.Poco.Poco::Crypto::DigestEngine is created and
...
public:
	...
	ECDSADigestEngine(const ECKey& key, const std::string &name);
		...
		///
		/// Throws a include.Poco.Poco.Poco::NotFoundException if no algorithm with the given name exists.
...
} } // namespace include.Poco.Poco.Poco::Crypto
